...
public:           
	...
	Node previousPointer( Node* pNode, Node* pTargetNode);
...
template<class NodeType, class ArcType>
GraphNode<NodeType, ArcType> Graph<NodeType, ArcType>::previousPointer( Node* pStartNode, Node* pTargetNode){
	if( pStartNode != 0 ) {
		queue<Node*> nodeQueue;     
		// place the first node on the queue, and mark it.
		nodeQueue.push( pStartNode );
		pStartNode->setMarked(true);
